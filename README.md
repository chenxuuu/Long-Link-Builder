# 长链接生成器 / Long-Link-Builder

---

可以把短链接变成特别特别特别长的链接

you can make your url links be longer

演示网址 / demo：http://biubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiu.com/

（光域名就挺长）



例如 / example：

输入 / input：http://baidu.com/

输出 / output：
https://biubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiubiu.

---

# 食用方法 / how to use

clone整个项目，把文件里的`biubiu....com`网址全部换成你自己的网址

clone this project to your local folder, change all url like `biubiu....com` to your website's url

扔到服务器上，无需数据库

you can run this without database

设置伪静态，nginx如下(apache自己照着写吧233)：

example nginx rewrite config(you can use apache based on this config):

```nginx
location / {
    if (!-e $request_filename) {
        rewrite ^(.*)/mirror(.*)$ $1/mirror.php?dat=$2 last;
        #mirror

        rewrite ^/(.*) /lj.php?dat=$1 last;
        #new
    }
}
```

---

欢迎pr/star

welcome to pull request or star
